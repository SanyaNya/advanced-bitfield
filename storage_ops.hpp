...
template<
    typename int_type, std::size_t OFFSET, 
    std::size_t BEGIN, std::size_t END, std::size_t ... Tail>
struct merge_parts_from_src
{
    static constexpr int_type f(int_type src) noexcept
    {
        return 
            (shift_to<OFFSET, BEGIN>(src) & 
             mask<int_type, BEGIN, END>::f()) | 
            merge_parts_from_src<
                int_type, OFFSET + (END-BEGIN), Tail...>::f(src);
    }
};
...
constexpr 
int_type set_storage(int_type storage, int_type src) noexcept
{
    return 
        (storage & ~mask<int_type, Parts...>::f()) | 
...
struct merge_parts_from_storage
{
    static constexpr int_type f(int_type storage) noexcept
    {
        return 
            ...
            merge_parts_from_storage<
                int_type, OFFSET + (END-BEGIN), Tail...>::f(storage);
    }
};
