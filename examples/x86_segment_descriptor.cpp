...
namespace x86{ namespace Segment
{
...
public:
    ...
    constexpr Access(
        readable_b read, conforming_privilege_b conf, privilege_b privil) 
            noexcept : 
                ABF_CTOR(bf, 
                    accessed_b(0),
                    read,
...
public:
    ...
    constexpr Access(
        writable_b write, direction_b dir, privilege_b privil)
            noexcept
                : ABF_CTOR(bf, 
                    accessed_b(0),
                    write,
...
template<Type type>
class Descriptor : 
    public abf::bitfield<Descriptor<type>, std::uint64_t>
{
...
{
    std::cout 
        << "Code segment descriptor representation: 0x"
        << std::setfill('0') << std::setw(16) << std::right << std::hex
        << static_cast<std::uint64_t>(gdt[1]) << "\n"

        << "Data segment descriptor representation: 0x"
        << std::setfill('0') << std::setw(16) << std::right << std::hex
        << static_cast<std::uint64_t>(gdt[2]) << "\n";
}
