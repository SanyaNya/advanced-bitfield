...
public:
  ...
  // ABF_FIELD(BASE_BITFIELD_CLASS, FIELD_TYPE, FIELD_NAME, PART0_BEGIN_BIT,
  // PART0_END_BIT, [PART1_BEGIN_BIT, PART1_END_BIT]...)
...
int main()
{
  ...
  std::cout << std::hex << "s representation: " << static_cast<std::uint32_t>(s)
            << "\n"
            << "s.f1() = " << s.f1() << "\n"
            << "s.n1() = " << s.n1() << "\n"
            << "s.n2() = " << s.n2() << "\n"
